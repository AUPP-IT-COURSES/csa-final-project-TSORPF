...
theta = -30
end = getPosOnCircumference(theta, origin)
arct = toRadian(theta)
...
def get_velocity_input():
    ...
    while input_active:
        ...
        win.fill(BLACK)
        input_text = font.render("Enter Velocity (m/s): " + velocity_input, True, WHITE)
        win.blit(input_text, (20, 20))

        if input_active:
            error_text = font.render(error_message, True, RED)
            win.blit(error_text, (20, 50))
...
while running:
    ...
    coordinateX = font.render(f"x", True, WHITE)
    coordinateY = font.render(f"y", True, WHITE)
    win.blit(title, (90, 30))
    win.blit(fpstext, (20, 400))
    win.blit(thetatext, (20, 420))
    win.blit(degreetext, (origin[0]+38, origin[1]-20))
    win.blit(coordinateX, (270, 345))
    win.blit(coordinateY, (7, 80))
